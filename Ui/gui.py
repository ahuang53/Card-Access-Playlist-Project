...
def show_info(selection_callback, prompt_font):
    search1 = dpg.get_value("search_bar_1")
    search2 = dpg.get_value("search_bar_2")
    message = "Is this correct?" #If statement to become an erorr statement if needed
    ...
    with dpg.mutex():
        ...
        with dpg.window(label = title, modal=True, no_close=True, width=480, height=270) as modal_id:
            m1 = dpg.add_text(message)
            dpg.bind_item_font(m1 , prompt_font)
            #if(search result = error): add text to be BIG ERROR, dont add buttons 
            #if(search result = found): add to message, {full title} , and depending on yes or no, save it to playlist
...
def resume_play(sender, app_data):
    print("resumed")

#Callback to stop the playlist
...
def setup_ui():
    ...
    with dpg.font_registry():
        regular_font = dpg.add_font("/mnt/c/users/andyh/Documents/Card-Access-Playlist-Project/fonts/Figtree-Regular.ttf", 60)
        prompt_font =  dpg.add_font("/mnt/c/users/andyh/Documents/Card-Access-Playlist-Project/fonts/Figtree-Regular.ttf", 50)
        bigger_font = dpg.add_font("/mnt/c/users/andyh/Documents/Card-Access-Playlist-Project/fonts/Figtree-Regular.ttf", 72)
    ...
    with dpg.window(tag = "Primary_Window", width=800, height=600):
        ...
        with dpg.group(tag = "play_pg", horizontal = False):
            ...
            with dpg.group(horizontal = True):
                ...
                dpg.add_text("Status: Playing...", tag = "status") #Add playing / stopped logic
            dpg.add_spacer(height = 20)
            with dpg.group(horizontal = True):
                ...
                dpg.add_spacer(width = 20)
                dpg.add_button(label="Play", tag = "playback_button", width=280, height=128, callback = resume_play, )
                dpg.bind_item_font("playback_button",bigger_font)
                dpg.add_spacer(width = 20)
                dpg.add_button(label="Stop", tag = "stop_play_button", width=280, height=128, callback = stop_play, )
